...
export function AppHeader({
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 items-center gap-2 sm:gap-4 border-b bg-background px-2 sm:px-4 md:px-6 w-full">
        <Button
          variant="ghost"
          size="icon"
        ...
        <Link href="/app/dashboard" className="flex items-center gap-2 font-semibold flex-shrink-0">
          <Briefcase className="h-6 w-6 text-primary" />
          <span className="text-lg hidden sm:inline">AIConstruct</span>
        </Link>
        <div className="flex-1 flex items-center justify-end gap-2 md:gap-4">
          <Button
            variant="outline"
            className="relative h-9 w-full justify-start rounded-md px-3 text-sm text-muted-foreground sm:w-64 sm:pr-12 md:w-80 lg:w-96 hidden xs:flex"
              ...
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem asChild>
                  <Link href="/app/knowledge">
                    <BookOpenText className="mr-2 h-4 w-4" /> Knowledge
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
